...
def feeds_status(current_user: str = Depends(get_current_user)):
    ...
    from services.cve_enrichment import (
        KEV_CACHE_PATH, EPSS_CACHE_PATH, CACHE_TTL_HOURS, _load_json,
        get_kev_meta, get_epss_meta,
    )
    from datetime import datetime, timezone
    ...
    kev_meta  = get_kev_meta()
    epss_meta = get_epss_meta()

    return {
        "ttl_hours": CACHE_TTL_HOURS,  # conservé pour compat — voir kev.ttl_hours/epss.ttl_hours (réglages indépendants)
        "kev": {
            ...
            "ttl_hours":       kev_meta["ttl_hours"],
            "fresh":           kev_meta["cache_fresh"],
        },
        "epss": {
            ...
            "ttl_hours":  epss_meta["ttl_hours"],
            "fresh":      epss_meta["cache_fresh"],
        },
    }
...
def feeds_refresh(current_user: str = Depends(get_admin_user)):
    ...
    def event_stream():
        ...
        from services.cve_enrichment import refresh_kev, get_kev_meta, refresh_epss_bulk
        ...
        try:
            ok, count = refresh_epss_bulk()
            if ok:
                yield emit(f"[OK] EPSS mis a jour — {count} scores charges", "success")
                audit_log("FEEDS_REFRESH", current_user, "SUCCESS",
                          detail=f"KEV + EPSS ({count} scores)")
            else:
                yield emit("[WARN] EPSS — impossible de recuperer les scores (cache conserve)", "warning")
        except Exception as e:
