...
def _get_ttl_hours(setting_key: str) -> int:
    """
    Lit le TTL configuré dans settings.json["security"][setting_key] (ex.
    "kev_ttl_hours"/"epss_ttl_hours", page Settings → Bases de sécurité).
    Repli sur CACHE_TTL_HOURS si absent des settings ou si get_settings()
    échoue (ex. contexte hors requête).
    """
    try:
        from services.settings import get_settings
        return int(get_settings().get("security", {}).get(setting_key, CACHE_TTL_HOURS))
    except Exception:
...
def get_kev_set() -> set[str]:
    ...
    Utilise le cache si frais (TTL configurable via settings.security.kev_ttl_hours),
    sinon tente une mise à jour (graceful fallback).
    """
    if not _cache_fresh(KEV_CACHE_PATH, ttl_hours=_get_ttl_hours("kev_ttl_hours")):
        refresh_kev()
...
def get_kev_meta() -> dict:
    """Retourne les métadonnées du cache KEV (date, total, TTL configuré)."""
    cached = _load_json(KEV_CACHE_PATH)
    ttl_hours = _get_ttl_hours("kev_ttl_hours")
    return {
        "total": cached.get("total", 0),
        "fetched_at": cached.get("fetched_at"),
        "catalog_version": cached.get("catalog_version", ""),
        "ttl_hours": ttl_hours,
        "cache_fresh": _cache_fresh(KEV_CACHE_PATH, ttl_hours=ttl_hours),
    }
...
def refresh_epss_bulk(days: int = 30, limit: int = 10000) -> tuple[bool, int]:
    """
    Rafraîchit le cache EPSS en bloc (top `limit` scores les plus récents sur
    une fenêtre de `days` jours) — contrairement à get_epss_scores(), qui ne
    récupère que les CVE demandées explicitement à la volée. Utilisé par le
    refresh manuel (POST /security/feeds/refresh) et par le cron
    security_sync_daily, pour que le cache EPSS ne dépende plus uniquement
    d'un scan CVE ayant déjà eu lieu.

    Retourne (succès, nombre de scores chargés). Écrase le cache existant
    uniquement en cas de succès — un échec réseau conserve le dernier cache
    connu (comportement identique à refresh_kev()).
    """
    try:
        resp = _get_with_retry(EPSS_URL, params={"days": days, "limit": limit}, timeout=30)
        resp.raise_for_status()
        ...
        logger.info(f"[EPSS] Rafraîchissement en bloc — {len(scores)} scores chargés")
        return True, len(scores)
    except Exception as e:
        logger.warning(f"[EPSS] Rafraîchissement en bloc impossible : {e}")
        return False, 0
...
def get_epss_scores(cve_ids: list[str]) -> dict[str, dict]:
    ...
    scores = _load_epss_cache() if _cache_fresh(EPSS_CACHE_PATH, ttl_hours=_get_ttl_hours("epss_ttl_hours")) else {}
...
def get_epss_meta() -> dict:
    """Retourne les métadonnées du cache EPSS (date, nb de scores, TTL configuré)."""
    cached = _load_json(EPSS_CACHE_PATH)
    ttl_hours = _get_ttl_hours("epss_ttl_hours")
    return {
        "count": len(cached.get("scores", {})),
        "updated_at": cached.get("updated_at"),
        "ttl_hours": ttl_hours,
        "cache_fresh": _cache_fresh(EPSS_CACHE_PATH, ttl_hours=ttl_hours),
    }
